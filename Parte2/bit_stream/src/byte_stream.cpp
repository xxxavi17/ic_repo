...
void ByteStream::flush() {
	size_t n_bytes_to_write = m_buf_ptr - m_buf;		//calcula os bytes pendentes

	if(n_bytes_to_write != 0) { // If buf is not empty
		m_fs.write((char*)m_buf, n_bytes_to_write);		//escreve so os bytes validos
		m_buf_ptr = m_buf;		//reset do ponteiro
	}
}
...
void ByteStream::close() {
	if(not m_rw_status)
		this->flush();	//se esta no mdo escrita, força o flush antes de fechar
